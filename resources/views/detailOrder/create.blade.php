	      ...
	      <div class="col-md-12">
	        ...
	        <div class="box box-primary">
	          <div class="box-header with-border">
	            <h3 class="box-title">Add Detail Order</h3>
	          </div>

	          @include('layouts.messages')
	          <!-- /.box-header -->
	          <!-- form start -->
	          <form method="POST" action="{{ url('/detailOrder') }}">
				@csrf
	            <div class="box-body">
	            <div class="col-lg-offset-3 col-lg-6">
	              ...
	              <div class="form-group">
	                ...
	                <select class="form-control" id="order_id" name="order_id">
						...
						<option value="{{ $order->id }}" {{ ( old('order_id') == $order->id ) ? 'selected' : '' }}>{{ $order->id.' '.$order->member->name }} </option>
						@endforeach
					</select>
	              </div>
	              ...
	              <div class="form-group">
	                ...
	                <select class="form-control" id="detail_item_id" name="detail_item_id">
						...
						<option value="{{ $detailItem->id }}" {{ ( old('detail_item_id') == $detailItem->id ) ? 'selected' : '' }}>{{ $detailItem->item->name }}</option>
						@endforeach
					</select>
	              </div>
